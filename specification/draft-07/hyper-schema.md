...
<table class="header"><tbody><tr><td class="left">Internet Engineering Task Force</td><td class="right">H. Andrews, Ed.</td></tr><tr><td class="left">Internet-Draft</td><td class="right">Cloudflare, Inc.</td></tr><tr><td class="left">Intended status: Informational</td><td class="right">A. Wright, Ed.</td></tr><tr><td class="left">Expires: July 23, 2018</td><td class="right">January 19, 2018</td></tr></tbody></table>
...
*   2\. [Notational Conventions](#rfc.section.2)
*   3\. [Overview](#rfc.section.3)

*   3.1. [Terminology](#rfc.section.3.1)
*   3.2. [Functionality](#rfc.section.3.2)

*   4\. [Meta-Schemas and Output Schema](#rfc.section.4)
*   5\. [Schema Keywords](#rfc.section.5)

*   5.1. [base](#rfc.section.5.1)
*   5.2. [links](#rfc.section.5.2)

*   6\. [Link Description Object](#rfc.section.6)

*   6.1. [Link Context](#rfc.section.6.1)

*   6.1.1. [anchor](#rfc.section.6.1.1)
*   6.1.2. [anchorPointer](#rfc.section.6.1.2)

*   6.2. [Link Relation Type](#rfc.section.6.2)

*   6.2.1. [rel](#rfc.section.6.2.1)
*   6.2.2. ["self" Links](#rfc.section.6.2.2)
*   6.2.3. ["collection" and "item" Links](#rfc.section.6.2.3)
*   6.2.4. [Using Extension Relation Types](#rfc.section.6.2.4)

*   6.3. [Link Target](#rfc.section.6.3)

*   6.3.1. [href](#rfc.section.6.3.1)

*   6.4. [Adjusting URI Template Resolution](#rfc.section.6.4)

*   6.4.1. [templatePointers](#rfc.section.6.4.1)
*   6.4.2. [templateRequired](#rfc.section.6.4.2)

*   6.5. [Link Target Attributes](#rfc.section.6.5)

*   6.5.1. [title](#rfc.section.6.5.1)
*   6.5.2. [description](#rfc.section.6.5.2)
*   6.5.3. [targetMediaType](#rfc.section.6.5.3)
*   6.5.4. [targetSchema](#rfc.section.6.5.4)
*   6.5.5. [targetHints](#rfc.section.6.5.5)

*   6.6. [Link Input](#rfc.section.6.6)

*   6.6.1. [hrefSchema](#rfc.section.6.6.1)
*   6.6.2. [headerSchema](#rfc.section.6.6.2)
*   6.6.3. [Manipulating the Target Resource Representation](#rfc.section.6.6.3)
*   6.6.4. [Submitting Data for Processing](#rfc.section.6.6.4)

*   7\. [Implementation Requirements](#rfc.section.7)

*   7.1. [Link Discovery and Look-Up](#rfc.section.7.1)
*   7.2. [URI Templating](#rfc.section.7.2)

*   7.2.1. [Populating Template Data From the Instance](#rfc.section.7.2.1)
*   7.2.2. [Accepting Input for Template Data](#rfc.section.7.2.2)
*   7.2.3. [Encoding Data as Strings](#rfc.section.7.2.3)

*   7.3. [Providing Access to LDO Keywords](#rfc.section.7.3)
*   7.4. [Requests](#rfc.section.7.4)
*   7.5. [Responses](#rfc.section.7.5)
*   7.6. [Streaming Parsers](#rfc.section.7.6)

*   8\. [JSON Hyper-Schema and HTTP](#rfc.section.8)

*   8.1. [One Link Per Target and Relation Type](#rfc.section.8.1)
*   8.2. ["targetSchema" and HTTP](#rfc.section.8.2)
*   8.3. [HTTP POST and the "submission\*" keywords](#rfc.section.8.3)
*   8.4. [Optimizing HTTP Discoverability With "targetHints"](#rfc.section.8.4)
*   8.5. [Advertising HTTP Features With "headerSchema"](#rfc.section.8.5)
*   8.6. [Creating Resources Through Collections](#rfc.section.8.6)
*   8.7. [Content Negotiation and Schema Evolution](#rfc.section.8.7)

*   9\. [Examples](#rfc.section.9)

*   9.1. [Entry Point Links, No Templates](#rfc.section.9.1)
*   9.2. [Individually Identified Resources](#rfc.section.9.2)
*   9.3. [Submitting a Payload and Accepting URI Input](#rfc.section.9.3)
*   9.4. ["anchor", "base" and URI Template Resolution](#rfc.section.9.4)
*   9.5. [Collections](#rfc.section.9.5)

*   9.5.1. [Pagination](#rfc.section.9.5.1)
*   9.5.2. [Creating the First Item](#rfc.section.9.5.2)

*   10\. [Security Considerations](#rfc.section.10)

*   10.1. [Target Attributes](#rfc.section.10.1)
*   10.2. ["self" Links](#rfc.section.10.2)

*   11\. [Acknowledgments](#rfc.section.11)
*   12\. [References](#rfc.references)

*   12.1. [Normative References](#rfc.references.1)
*   12.2. [Informative References](#rfc.references.2)

*   Appendix A. [Using JSON Hyper-Schema in APIs](#rfc.appendix.A)

*   A.1. [Resource Evolution With Hyper-Schema](#rfc.appendix.A.1)
*   A.2. [Responses and Errors](#rfc.appendix.A.2)
*   A.3. [Static Analysis of an API's Hyper-Schemas](#rfc.appendix.A.3)
...
The primary mechanism introduced for specifying links is the Link Description Object (LDO), which is a serialization of the abstract link model defined in [RFC 8288, section 2](#RFC8288) \[RFC8288\].

This specification will use the concepts, syntax, and terminology defined by the [JSON Schema core](#json-schema) \[json-schema\] and [JSON Schema validation](#json-schema-validation) \[json-schema-validation\] specifications. It is advised that readers have a copy of these specifications.

# [2.](#rfc.section.2) Notational Conventions
...
If the instance is {"id": 1234}, and its base URI according to [RFC 3986 section 5.1](#RFC3986) \[RFC3986\], is "https://api.example.com/", then "https://api.example.com/thing/1234" is the resulting link's target URI.

# [3.1.](#rfc.section.3.1) Terminology

The terms "schema", "instance", and "meta-schema" are to be interpreted as defined in the [JSON Schema core specification](#json-schema) \[json-schema\].

The terms "applicable" and "attached" are to be interpreted as defined in [Section 3 of the JSON Schema validation specification](#json-schema-validation) \[json-schema-validation\].

The terms "link", "link context" (or "context"), "link target" (or "target"), and "target attributes" are to be interpreted as defined in [Section 2 of RFC 8288](#RFC8288) \[RFC8288\].

The term "user agent" is to be interpreted as defined in [Section 2.1 of RFC 7230](#RFC7230) \[RFC7230\], generalized to apply to any protocol that may be used in a hypermedia system rather than specifically being an HTTP client.
...
# [3.2.](#rfc.section.3.2) Functionality

A JSON Hyper-Schema implementation is able to take a hyper-schema, an instance, and in some cases client input, and produce a set of fully resolved valid links. As defined by [RFC 8288, section 2](#RFC8288) \[RFC8288\], a link consists of a context, a typed relation, a target, and optionally additional target attributes.
...
# [4.](#rfc.section.4) Meta-Schemas and Output Schema

The current URI for the JSON Hyper-Schema meta-schema is <[http://json-schema.org/draft-07/hyper-schema#](https://json-schema.org/draft-07/hyper-schema#)\>.

The [link description format](#ldo) \[ldo\] can be used without JSON Schema, and use of this format can be declared by referencing the normative link description schema as the schema for the data structure that uses the links. The URI of the normative link description schema is: <[http://json-schema.org/draft-07/links#](https://json-schema.org/draft-07/links#)\>.

JSON Hyper-Schema implementations are free to provide output in any format. However, a specific format is defined for use in the conformance test suite, which is also used to illustrate points in the ["Implementation Requirements"](#implementation) \[implementation\], and to show the output generated by [examples](#examples) \[examples\]. It is RECOMMENDED that implementations be capable of producing output in this format to facilitated testing. The URI of the JSON Schema describing the recommended output format is <[http://json-schema.org/draft-07/hyper-schema-output#](https://json-schema.org/draft-07/hyper-schema-output#)\>.

# [5.](#rfc.section.5) Schema Keywords

Hyper-schema keywords from all schemas that are applicable to a position in an instance, as defined by [Section 3 of JSON Schema validation](#json-schema-validation) \[json-schema-validation\], can be used with that instance.
...
# [6.](#rfc.section.6) [Link Description Object](#ldo)

A Link Description Object (LDO) is a serialization of the abstract link model defined in [RFC 8288, section 2](#RFC8288) \[RFC8288\]. As described in that document, a link consists of a context, a relation type, a target, and optionally target attributes. JSON Hyper-Schema's LDO provides all of these, along with additional features using JSON Schema to describe input for use with the links in various ways.
...
A Link Description Object MUST be an object, and the ["href"](#href) \[href\] and ["rel"](#rel) \[rel\] properties MUST be present. Each keyword is covered briefly in this section, with additional usage explanation and comprehensive examples given later in the document.

# [6.1.](#rfc.section.6.1) [Link Context](#context)

In JSON Hyper-Schema, the link's context resource is, by default, the sub-instance to which it is attached (as defined by [Section 3 of the JSON Schema validation specification](#json-schema-validation) \[json-schema-validation\]). This is often not the entire instance document. This default context can be changed using the keywords in this section.
...
Implementations MUST be able to construct the link context's URI, and (if necessary for full identification), a JSON Pointer in string representation form as per [RFC 6901, section 5](#RFC6901) \[RFC6901\] in place of a URI fragment. The process for constructing a URI based on a URI template is given in the [URI Templating](#uriTemplating) \[uriTemplating\] section.

# [6.1.1.](#rfc.section.6.1.1) [anchor](#anchor)

This property sets the context URI of the link. The value of the property is a [URI Template](#RFC6570) \[RFC6570\], and the resulting [URI-reference](#RFC3986) \[RFC3986\] MUST be resolved against the base URI of the instance.

The URI is computed from the provided URI template using the same process described for the ["href"](#href) \[href\] property, with the exception that ["hrefSchema"](#hrefSchema) \[hrefSchema\] MUST NOT be applied. Unlike target URIs, context URIs do not accept user input.

# [6.1.2.](#rfc.section.6.1.2) [anchorPointer](#anchorPointer)
...
While an alternate context with a known URI is best set with the ["anchor"](#anchor) \[anchor\] keyword, the lack of a fragment identifier syntax for application/json means that it is usually not possible to change the context within a JSON instance using a URI.
...
# [6.2.](#rfc.section.6.2) [Link Relation Type](#relationType)
...
# [6.2.1.](#rfc.section.6.2.1) [rel](#rel)
...
# [6.2.2.](#rfc.section.6.2.2) ["self" Links](#self)

A "self" link, as originally defined by [Section 4.2.7.2 of RFC 4287](#RFC4287) \[RFC4287\], indicates that the target URI identifies a resource equivalent to the link context. In JSON Hyper-Schema, a "self" link MUST be resolvable from the instance, and therefore "hrefSchema" MUST NOT be present.
...
# [6.2.3.](#rfc.section.6.2.3) ["collection" and "item" Links](#collectionAndItem)

[RFC 6573](#RFC6573) \[RFC6573\] defines and registers the "item" and "collection" link relation types. JSON Hyper-Schema imposes additional semantics on collection resources indicated by these types.
...
As such a method would correspond to JSON Hyper-Schema's data submission concept, the ["submissionSchema"](#submissionSchema) \[submissionSchema\] field for the link SHOULD be compatible with the schema of the representation of the collection's items, as indicated by the "item" link's target resource or the "self" link of the "collection" link's context resource.

# [6.2.4.](#rfc.section.6.2.4) [Using Extension Relation Types](#extensionRelationTypes)

When no registered relation (aside from "related") applies, users are encouraged to mint their own extension relation types, as described in [section 2.1.2 of RFC 8288](#RFC8288) \[RFC8288\]. The simplest approaches for choosing link relation type URIs are to either use a URI scheme that is already in use to identify the system's primary resources, or to use a human-readable, non-dereferenceable URI scheme such as ["tag", defined by RFC 4151](#RFC4151) \[RFC4151\].

Extension relation type URIs need not be dereferenceable, even when using a scheme that allows it.

# [6.3.](#rfc.section.6.3) [Link Target](#target)

The target URI template is used to identify the link's target, potentially making use of instance data. Additionally, with ["hrefSchema"](#hrefSchema) \[hrefSchema\], this template can identify a set of possible target resources to use based on client input. The full process of resolving the URI template, with or without client input, is covered in the [URI Templating](#uriTemplating) \[uriTemplating\] section.

# [6.3.1.](#rfc.section.6.3.1) [href](#href)

The value of the "href" link description property is a template used to determine the target URI of the related resource. The value of the instance property MUST be resolved as a [URI-reference](#RFC3986) \[RFC3986\] against the base URI of the instance.

This property is REQUIRED.

# [6.4.](#rfc.section.6.4) Adjusting URI Template Resolution

The keywords in this section are used when resolving all URI Templates involved in hyper-schema: "base", "anchor", and "href". See the [URI Templating](#uriTemplating) \[uriTemplating\] section for the complete template resolution algorithm.
...
# [6.4.2.](#rfc.section.6.4.2) [templateRequired](#templateRequired)
...
# [6.5.](#rfc.section.6.5) [Link Target Attributes](#targetAttributes)
...
# [6.5.2.](#rfc.section.6.5.2) description
...
# [6.5.3.](#rfc.section.6.5.3) targetMediaType

The value of this property represents the media type [RFC 2046](#RFC2046) \[RFC2046\], that is expected to be returned when fetching this resource. This property value MAY be a media range instead, using the same pattern defined in [RFC 7231, section 5.3.2 - HTTP "Accept" header](#RFC7231) \[RFC7231\].

This property is analogous to the "type" property of other link serialization formats. User agents MAY use this information to inform the interface they present to the user before the link is followed, but MUST NOT use this information in the interpretation of the resulting data. Instead, a user agent MUST use the media type given by the response for run-time interpretation. See the section on ["Security Concerns"](#security) \[security\] for a detailed examination of mis-use of "targetMediaType".

For protocols supporting content-negotiation, implementations MAY choose to describe possible target media types using protocol-specific information in ["headerSchema"](#headerSchema) \[headerSchema\]. If both protocol-specific information and "targetMediaType" are present, then the value of "targetMediaType" MUST be compatible with the protocol-specific information, and SHOULD indicate the media type that will be returned in the absence of content negotiation.
...
# [6.5.4.](#rfc.section.6.5.4) [targetSchema](#targetSchema)

This property provides a schema that is expected to describe the link target's representation. Depending on the protocol, the schema may or may not describe the request or response to any particular operation performed with the link. See the [JSON Hyper-Schema and HTTP](#HTTP) \[HTTP\] section for an in-depth discussion of how this keyword is used with HTTP.

# [6.5.5.](#rfc.section.6.5.5) [targetHints](#targetHints)
...
See ["JSON Hyper-Schema and HTTP"](#HTTP) \[HTTP\] for guidance on using this keyword with HTTP and analogous protocols.

# [6.6.](#rfc.section.6.6) [Link Input](#input)
...
# [6.6.1.](#rfc.section.6.6.1) [hrefSchema](#hrefSchema)

The value of the "hrefSchema" link description property MUST be a valid JSON Schema. This schema is used to validate user input or other user agent data for filling out the URI Template in ["href"](#href) \[href\].
...
# [6.6.2.](#rfc.section.6.6.2) [headerSchema](#headerSchema)
...
# [6.6.3.](#rfc.section.6.6.3) Manipulating the Target Resource Representation

In JSON Hyper-Schema, ["targetSchema"](#targetSchema) \[targetSchema\] supplies a non-authoritative description of the target resource's representation. A client application can use "targetSchema" to structure input for replacing or modifying the representation, or as the base representation for building a patch document based on a patch media type.
...
"targetSchema" is not intended to describe link operation responses, except when the response semantics indicate that it is a representation of the target resource. In all cases, the schema indicated by the response itself is authoritative. See ["JSON Hyper-Schema and HTTP"](#HTTP) \[HTTP\] for detailed examples.

# [6.6.4.](#rfc.section.6.6.4) Submitting Data for Processing

The ["submissionSchema"](#submissionSchema) \[submissionSchema\] and ["submissionMediaType"](#submissionMediaType) \[submissionMediaType\] keywords describe the domain of the processing function implemented by the target resource. Otherwise, as noted above, the submission schema and media type are ignored for operations to which they are not relevant.

# [6.6.4.1.](#rfc.section.6.6.4.1) [submissionMediaType](#submissionMediaType)

If present, this property indicates the media type format the client application and user agent should use for the request payload described by ["submissionSchema"](#submissionSchema) \[submissionSchema\].
...
# [6.6.4.2.](#rfc.section.6.6.4.2) [submissionSchema](#submissionSchema)
...
Note that the requirements around how an implementation MUST recognize "self", "collection", and "item" links are thoroughly covered in the [link relation type](#relationType) \[relationType\] section and are not repeated here.
...
# [7.2.1.](#rfc.section.7.2.1) Populating Template Data From the Instance
...
# [7.2.2.](#rfc.section.7.2.2) Accepting Input for Template Data
...
# [7.2.3.](#rfc.section.7.2.3) Encoding Data as Strings
...
# [7.3.](#rfc.section.7.3) Providing Access to LDO Keywords
...
Other responses, including error responses, complex redirections, and processing status representations SHOULD also link to their own schemas and use appropriate media types (e.g. ["application/problem+json"](#RFC7807) \[RFC7807\] for errors). Certain errors might not link a schema due to being generated by an intermediary that is not aware of hyper-schema, rather than by the origin.
...
Statically mapping all possible responses and their schemas at design time is outside of the scope of JSON Hyper-Schema, but may be within the scope of other JSON Schema vocabularies which build on hyper-schema (see [Appendix A.3](#staticAnalysis)).

# [7.6.](#rfc.section.7.6) [Streaming Parsers](#streaming)
...
# [8.](#rfc.section.8) [JSON Hyper-Schema and HTTP](#HTTP)
...
[Section 11 of the JSON Schema core specification](#json-schema) \[json-schema\] provides guidance on linking instances in a hypermedia system to their schemas. This may be done with network-accessible schemas, or may simply identify schemas which were pre-packaged within the client application. JSON Hyper-Schema intentionally does not constrain this mechanism, although it is RECOMMENDED that the techniques outlined in the core specification be used to whatever extent is possible.

# [8.1.](#rfc.section.8.1) One Link Per Target and Relation Type

Link Description Objects do not directly indicate what operations, such as HTTP methods, are supported by the target resource. Instead, operations should be inferred primarily from link [relation types](#rel) \[rel\] and URI schemes.
...
# [8.2.](#rfc.section.8.2) ["targetSchema" and HTTP](#targetHTTP)
...
# [8.5.](#rfc.section.8.5) Advertising HTTP Features With "headerSchema"

Schemas SHOULD be written to describe JSON serializations that follow guidelines established by the work in progress ["A JSON Encoding for HTTP Header Field Values"](#I-D.reschke-http-jfv) \[I-D.reschke-http-jfv\] Approaches shown in that document's examples SHOULD be applied to other similarly structured headers wherever possible.
...
# [8.6.](#rfc.section.8.6) Creating Resources Through Collections

When using HTTP, or a protocol such as CoAP that is explicitly analogous to HTTP, this is done by POST-ing a representation of the individual resource to be created to the collection resource. The process for recognizing collection and item resources is described in [Section 6.2.3](#collectionAndItem).

# [8.7.](#rfc.section.8.7) Content Negotiation and Schema Evolution
...
# [9.](#rfc.section.9) [Examples](#examples)
...
Most other keywords are either straightforward ("title" and "description"), apply validation to specific sorts of input, requests, or responses, or have protocol-specific behavior. Examples demonstrating HTTP usage are available in [an Appendix](#HTTP) \[HTTP\].

# [9.1.](#rfc.section.9.1) [Entry Point Links, No Templates](#entryPoint)
...
# [9.2.](#rfc.section.9.2) Individually Identified Resources
...
# [9.3.](#rfc.section.9.3) [Submitting a Payload and Accepting URI Input](#mailto)
...
The "submissionSchema" and "submissionMediaType" fields are for describing payloads that are not representations of the target resource. When used with "http(s)://" URIs, they generally refer to a POST request payload, as seen in the [appendix on HTTP usage](#HTTP) \[HTTP\].

In this case, we use a "mailto:" URI, which, per [RFC 6068, Section 3"](#RFC6068) \[RFC6068\], does not provide any operation for retrieving a resource. It can only be used to construct a message for sending. Since there is no concept of a retrievable, replaceable, or deletable target resource, "targetSchema" and "targetMediaType" are not used. Non-representation payloads are described by "submissionSchema" and "submissionMediaType".
...

{
    ...
    "rel": "author",
    "hrefInputTemplates": \[
      "mailto:[\[email protected\]](/cdn-cgi/l/email-protection)?subject={title}{&cc}"
    \],
    "hrefPrepopulatedInput": {
...
"mailto:[\[email protected\]](/cdn-cgi/l/email-protection)?subject=The%20Awesome%20Thing"

Change "title" to "your work":

"mailto:[\[email protected\]](/cdn-cgi/l/email-protection)?subject=your%20work"

Change title and add a "cc" of "[\[email protected\]](/cdn-cgi/l/email-protection)":

"mailto:[\[email protected\]](/cdn-cgi/l/email-protection)?subject=your%20work&[\[email protected\]](/cdn-cgi/l/email-protection)"

# [9.4.](#rfc.section.9.4) "anchor", "base" and URI Template Resolution
...
# [9.5.](#rfc.section.9.5) Collections
...
There are three "self" links, one for the collection, and one for each item in the "elements" array. The item "self" links are defined in the individual "thing" schema which is referenced with "$ref". The three links can be distinguished by their context or attachment pointers. We will revisit the "submissionSchema" of the collection's "self" link in [Section 9.5.2](#firstItem).
...
# [9.5.1.](#rfc.section.9.5.1) Pagination
...
Let's add a link for this collection to the entry point schema ([Section 9.1](#entryPoint)), including pagination input in order to allow client applications to jump directly to a specific page. Recall that the entry point schema consists only of links, therefore we only show the newly added link:
...
# [9.5.2.](#rfc.section.9.5.2) [Creating the First Item](#firstItem)
...
# [10.2.](#rfc.section.10.2) "self" Links
...
# [11.](#rfc.section.11) Acknowledgments
...
<table><tbody><tr><td class="reference"><b id="RFC2119">[RFC2119]</b></td><td class="top"><a>Bradner, S.</a>, "<a href="http://tools.ietf.org/html/rfc2119">Key words for use in RFCs to Indicate Requirement Levels</a>", BCP 14, RFC 2119, DOI 10.17487/RFC2119, March 1997.</td></tr><tr><td class="reference"><b id="RFC3986">[RFC3986]</b></td><td class="top"><a>Berners-Lee, T.</a>, <a>Fielding, R.</a> and <a>L. Masinter</a>, "<a href="http://tools.ietf.org/html/rfc3986">Uniform Resource Identifier (URI): Generic Syntax</a>", STD 66, RFC 3986, DOI 10.17487/RFC3986, January 2005.</td></tr><tr><td class="reference"><b id="RFC4287">[RFC4287]</b></td><td class="top"><a>Nottingham, M.</a> and <a>R. Sayre</a>, "<a href="http://tools.ietf.org/html/rfc4287">The Atom Syndication Format</a>", RFC 4287, DOI 10.17487/RFC4287, December 2005.</td></tr><tr><td class="reference"><b id="RFC6570">[RFC6570]</b></td><td class="top"><a>Gregorio, J.</a>, <a>Fielding, R.</a>, <a>Hadley, M.</a>, <a>Nottingham, M.</a> and <a>D. Orchard</a>, "<a href="http://tools.ietf.org/html/rfc6570">URI Template</a>", RFC 6570, DOI 10.17487/RFC6570, March 2012.</td></tr><tr><td class="reference"><b id="RFC6573">[RFC6573]</b></td><td class="top"><a>Amundsen, M.</a>, "<a href="http://tools.ietf.org/html/rfc6573">The Item and Collection Link Relations</a>", RFC 6573, DOI 10.17487/RFC6573, April 2012.</td></tr><tr><td class="reference"><b id="RFC6901">[RFC6901]</b></td><td class="top"><a>Bryan, P.</a>, <a>Zyp, K.</a> and <a>M. Nottingham</a>, "<a href="http://tools.ietf.org/html/rfc6901">JavaScript Object Notation (JSON) Pointer</a>", RFC 6901, DOI 10.17487/RFC6901, April 2013.</td></tr><tr><td class="reference"><b id="RFC8288">[RFC8288]</b></td><td class="top"><a>Nottingham, M.</a>, "<a href="http://tools.ietf.org/html/rfc8288">Web Linking</a>", RFC 8288, DOI 10.17487/RFC8288, October 2017.</td></tr><tr><td class="reference"><b id="relative-json-pointer">[relative-json-pointer]</b></td><td class="top"><a>Luff, G.</a> and <a title="Cloudflare, Inc.">H. Andrews</a>, "<a href="http://tools.ietf.org/html/draft-handrews-relative-json-pointer-01">Relative JSON Pointers</a>", Internet-Draft draft-handrews-relative-json-pointer-01, January 2018.</td></tr><tr><td class="reference"><b id="json-schema">[json-schema]</b></td><td class="top"><a>Wright, A.</a> and <a title="Cloudflare, Inc.">H. Andrews</a>, "<a href="http://tools.ietf.org/html/draft-handrews-json-schema-00">JSON Schema: A Media Type for Describing JSON Documents</a>", Internet-Draft draft-handrews-json-schema-00, November 2017.</td></tr><tr><td class="reference"><b id="json-schema-validation">[json-schema-validation]</b></td><td class="top"><a>Wright, A.</a>, <a title="Cloudflare, Inc.">Andrews, H.</a> and <a>G. Luff</a>, "<a href="http://tools.ietf.org/html/draft-handrews-json-schema-validation-00">JSON Schema Validation: A Vocabulary for Structural Validation of JSON</a>", Internet-Draft draft-handrews-json-schema-validation-00, November 2017.</td></tr></tbody></table>

# [12.2.](#rfc.references.2) Informative References

<table><tbody><tr><td class="reference"><b id="RFC2046">[RFC2046]</b></td><td class="top"><a>Freed, N.</a> and <a>N. Borenstein</a>, "<a href="http://tools.ietf.org/html/rfc2046">Multipurpose Internet Mail Extensions (MIME) Part Two: Media Types</a>", RFC 2046, DOI 10.17487/RFC2046, November 1996.</td></tr><tr><td class="reference"><b id="RFC4151">[RFC4151]</b></td><td class="top"><a>Kindberg, T.</a> and <a>S. Hawke</a>, "<a href="http://tools.ietf.org/html/rfc4151">The 'tag' URI Scheme</a>", RFC 4151, DOI 10.17487/RFC4151, October 2005.</td></tr><tr><td class="reference"><b id="RFC5789">[RFC5789]</b></td><td class="top"><a>Dusseault, L.</a> and <a>J. Snell</a>, "<a href="http://tools.ietf.org/html/rfc5789">PATCH Method for HTTP</a>", RFC 5789, DOI 10.17487/RFC5789, March 2010.</td></tr><tr><td class="reference"><b id="RFC6068">[RFC6068]</b></td><td class="top"><a>Duerst, M.</a>, <a>Masinter, L.</a> and <a>J. Zawinski</a>, "<a href="http://tools.ietf.org/html/rfc6068">The 'mailto' URI Scheme</a>", RFC 6068, DOI 10.17487/RFC6068, October 2010.</td></tr><tr><td class="reference"><b id="RFC7230">[RFC7230]</b></td><td class="top"><a>Fielding, R.</a> and <a>J. Reschke</a>, "<a href="http://tools.ietf.org/html/rfc7230">Hypertext Transfer Protocol (HTTP/1.1): Message Syntax and Routing</a>", RFC 7230, DOI 10.17487/RFC7230, June 2014.</td></tr><tr><td class="reference"><b id="RFC7231">[RFC7231]</b></td><td class="top"><a>Fielding, R.</a> and <a>J. Reschke</a>, "<a href="http://tools.ietf.org/html/rfc7231">Hypertext Transfer Protocol (HTTP/1.1): Semantics and Content</a>", RFC 7231, DOI 10.17487/RFC7231, June 2014.</td></tr><tr><td class="reference"><b id="RFC7807">[RFC7807]</b></td><td class="top"><a>Nottingham, M.</a> and <a>E. Wilde</a>, "<a href="http://tools.ietf.org/html/rfc7807">Problem Details for HTTP APIs</a>", RFC 7807, DOI 10.17487/RFC7807, March 2016.</td></tr><tr><td class="reference"><b id="I-D.reschke-http-jfv">[I-D.reschke-http-jfv]</b></td><td class="top"><a>Reschke, J.</a>, "<a href="http://tools.ietf.org/html/draft-reschke-http-jfv-06">A JSON Encoding for HTTP Header Field Values</a>", Internet-Draft draft-reschke-http-jfv-06, June 2017.</td></tr></tbody></table>

# [Appendix A.](#rfc.appendix.A) [Using JSON Hyper-Schema in APIs](#apis)
...
# [A.1.](#rfc.appendix.A.1) Resource Evolution With Hyper-Schema
...
# [A.2.](#rfc.appendix.A.2) [Responses and Errors](#responses)

Because a hyper-schema represents a single resource at a time, it does not provide for an enumeration of all possible responses to protocol operations performed with links. Each response, including errors, is considered its own (possibly anonymous) resource, and should identify its own hyper-schema, and optionally use an appropriate media type such as [RFC 7807's "application/problem+json"](#RFC7807) \[RFC7807\], to allow the user agent or client application to interpret any information that is provided beyond the protocol's own status reporting.

# [A.3.](#rfc.appendix.A.3) [Static Analysis of an API's Hyper-Schemas](#staticAnalysis)
...
Henry Andrews (editor) Andrews Cloudflare, Inc. San Francisco, CA USA EMail: [\[email protected\]](/cdn-cgi/l/email-protection#4e262b203c370e2d22213b2a28222f3c2b602d2123)

Austin Wright (editor) Wright EMail: [\[email protected\]](/cdn-cgi/l/email-protection#72131313321008140a5c1c1706)