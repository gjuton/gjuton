...
- [x] A public class in `se.plilja.jsonschemagen.api` accepts a JSON schema
      string and produces a JSON string
- [x] An optional seed can be provided to make output reproducible — two
      calls with the same schema and seed produce identical output
- [x] The public API does not expose any internal or third-party types
- [x] The module still compiles and `mvn clean verify` passes
