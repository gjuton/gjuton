...
<table class="header"><tbody><tr><td class="left">Internet Engineering Task Force</td><td class="right">A. Wright, Ed.</td></tr><tr><td class="left">Internet-Draft</td><td class="right"></td></tr><tr><td class="left">Intended status: Informational</td><td class="right">H. Andrews, Ed.</td></tr><tr><td class="left">Expires: October 23, 2017</td><td class="right">Cloudflare, Inc.</td></tr><tr><td class="left"></td><td class="right">April 21, 2017</td></tr></tbody></table>
...
*   3.2. [Hypermedia and Linking](#rfc.section.3.2)

*   4\. [Definitions](#rfc.section.4)

*   4.1. [JSON Document](#rfc.section.4.1)
*   4.2. [Instance](#rfc.section.4.2)
*   4.3. [Instance equality](#rfc.section.4.3)
*   4.4. [JSON Schema documents](#rfc.section.4.4)
*   4.5. [Root schema and subschemas](#rfc.section.4.5)
...
*   8\. [Schema references with $ref](#rfc.section.8)
*   9\. [Base URI and dereferencing](#rfc.section.9)

*   9.1. [Initial base URI](#rfc.section.9.1)
*   9.2. [The "$id" keyword](#rfc.section.9.2)

*   9.2.1. [Internal references](#rfc.section.9.2.1)
*   9.2.2. [External references](#rfc.section.9.2.2)

*   10\. [Usage for hypermedia](#rfc.section.10)

*   10.1. [Linking to a schema](#rfc.section.10.1)
*   10.2. [Describing a profile of JSON](#rfc.section.10.2)
...
# [3.2.](#rfc.section.3.2) Hypermedia and Linking
...
# [4.3.](#rfc.section.4.3) Instance equality
...
# [4.4.](#rfc.section.4.4) JSON Schema documents
...
# [4.5.](#rfc.section.4.5) Root schema and subschemas
...
Additionally, the "application/schema+json" media type supports two fragment identifier structures: plain names and JSON Pointers. The use of JSON Pointers as URI fragment identifiers is described in [RFC 6901](#RFC6901) \[RFC6901\]. Fragment identifiers matching the JSON Pointer syntax, including the empty string, MUST be interpreted as JSON Pointer fragment identifiers.

Per the W3C's [best practices for fragment identifiers](#W3C.WD-fragid-best-practices-20121025) \[W3C.WD-fragid-best-practices-20121025\], plain name fragment identifiers are reserved for referencing locally named schemas. All fragment identifiers that do not match the JSON Pointer syntax MUST be interpreted as plain name fragment identifiers.

Defining and referencing a plain name fragment identifier in the ["$id" keyword](#id-keyword) \[id-keyword\] section.
...
# [8.](#rfc.section.8) Schema references with $ref
...
# [9.](#rfc.section.9) Base URI and dereferencing

# [9.1.](#rfc.section.9.1) Initial base URI
...
# [9.2.](#rfc.section.9.2) [The "$id" keyword](#id-keyword)
...
The schemas at the following URI-encoded [JSON Pointers](#RFC6901) \[RFC6901\] (relative to the root schema) have the following base URIs, and are identifiable by either URI in accordance with Section [5](#fragments) above:
...
# [9.2.1.](#rfc.section.9.2.1) Internal references
...
# [9.2.2.](#rfc.section.9.2.2) External references
...
# [10.](#rfc.section.10) Usage for hypermedia

JSON has been adopted widely by HTTP servers for automated APIs and robots. This section describes how to enhance processing of JSON documents in a more RESTful manner when used with protocols that support media types and [Web linking](#RFC5988) \[RFC5988\].

# [10.1.](#rfc.section.10.1) Linking to a schema

It is RECOMMENDED that instances described by a schema/profile provide a link to a downloadable JSON Schema using the link relation "describedby", as defined by [Linked Data Protocol 1.0, section 8.1](#W3C.REC-ldp-20150226) \[W3C.REC-ldp-20150226\].

In HTTP, such links can be attached to any response using the [Link header](#RFC5988) \[RFC5988\]. An example of such a header would be:

Link: <http://example.com/my-hyper-schema#>; rel="describedby"

                    

# [10.2.](#rfc.section.10.2) [Describing a profile of JSON](#profile)

Instances MAY specify a "profile" as described in [The 'profile' Link Relation](#RFC6906) \[RFC6906\]. When used as a media-type parameter, HTTP servers gain the ability to perform Content-Type Negotiation based on profile. The media-type parameter MUST be a whitespace-separated list of URIs (i.e. relative references are invalid).
...
Henry Andrews (editor) Andrews Cloudflare, Inc. EMail: [\[email protected\]](/cdn-cgi/l/email-protection#78101d160a01381b14170d1c1e14190a1d561b1715)